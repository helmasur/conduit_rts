...
static func handle_collect_state(unit, delta: float) -> void:
	match unit.mode:
		UnitShared.ActionMode.COLLECT_STARTING:
			unit.mode_timer -= delta
			if unit.mode_timer <= 0:
				unit.mode = UnitShared.ActionMode.COLLECTING
				unit.mode_timer = 0

		UnitShared.ActionMode.COLLECTING:
		...
		UnitShared.ActionMode.COLLECT_STOPPING:
			unit.mode_timer -= delta
			if unit.mode_timer <= 0:
